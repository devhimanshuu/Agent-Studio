...
describe("Provider selection (LLMProviderConfig)", () => {
  ...
  it("selects only OpenRouter models when only the OpenRouter key is set", () => {
    ...
    expect(router.isConfigured()).toBe(true);
    expect(countConfiguredModels({ openRouterApiKey: "sk-or-v1-test" })).toBe(OPENROUTER_CHAT_MODELS.length);
    expect(listConfiguredModels({ openRouterApiKey: "sk-or-v1-test" })).toEqual(OPENROUTER_CHAT_MODELS.map((m) => m.model));
  });
  ...
  it("selects both vendors when both keys are set (Groq first)", () => {
    ...
    expect(models[GROQ_FREE_MODELS.length]).toBe(OPENROUTER_CHAT_MODELS[0].model); // then best OpenRouter model
  });
