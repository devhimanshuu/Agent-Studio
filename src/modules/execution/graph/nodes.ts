...
/** True when the given step must pause for human approval. */
export function stepRequiresApproval(step: { action: string; requiresApproval: boolean }, version: SkillVersionDTO | null): boolean {
  if (step.requiresApproval) return true;
  return Boolean(version?.actionsRequiringApproval?.includes(step.action));
}
...
export const toolSelectionNode: GraphNode = async (state, config) => {
  ...
  return { currentStep: nextIndex + 1 };
};
...
export const toolExecutionNode: GraphNode = async (state, config) => {
  ...
  const requiresApproval = stepRequiresApproval(step, version);
  ...
  try {
    const output = await withRetries(
      () => runtime.toolRegistry.executeTool(step.toolName, step.input),
      {
        attempts: 2,
        delayMs: 0,
        // Retry transient tool failures once. Domain errors (ExecutionError,
        // e.g. a blocked/unauthorized action) must fail fast — but a plain
        // transient failure (vendor/network hiccup inside the tool) is worth
        // one retry. Previously `attempts: 2` was dead: the default predicate
        // only retries errors flagged `retryable`, which tool errors never are.
        isRetryable: (error) => !(error instanceof ExecutionError),
      }
    );
 ...
 * Resolution (approve/reject) is the next phase; here the run parks in
 * PAUSED_FOR_APPROVAL and the graph ends.
 */
