    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-indigo-950/80 pb-6">
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded border border-indigo-500/40 bg-indigo-950/40 text-xs font-mono text-indigo-200 hover:border-indigo-400 hover:text-white transition-all"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono">
        {widgets.map((w, i) => (
          <div
            key={w.label}
            className="p-5 rounded border border-indigo-900/50 bg-[#0a0a0a]/80 space-y-2 hover:border-indigo-500/50 hover:-translate-y-0.5 transition-all duration-300"
            style={{ animationDelay: `${i * 60}ms` }}
          >
            <div className={`flex items-center justify-between text-xs tracking-wider uppercase ${w.tone}`}>
              <span>{w.label}</span>
              {w.icon}
            </div>
            <div className="text-3xl font-pixel text-slate-100">{w.value}</div>
            <p className="text-[11px] text-slate-400">{w.sub}</p>
          </div>
        ))}
      </div>
      ...
      {metrics && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 font-mono">
          {Object.entries(metrics.executionsByStatus).map(([status, count]) => (
            <div key={status} className="rounded border border-indigo-950/60 bg-black/40 px-3 py-2.5 flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 font-mono">
        <div className="rounded border border-indigo-900/40 bg-[#0a0a0a]/60 p-5 space-y-3">
          <div className="text-[10px] uppercase tracking-widest text-indigo-400/80 flex items-center gap-1.5 border-b border-indigo-950/60 pb-2">
            ...
            <ul className="space-y-2">
              {metrics.mostUsedSkills.map((s) => (
                <li key={s.skillName} className="flex items-center justify-between gap-3">
                  <span className="text-xs text-slate-300 truncate">{s.skillName}</span>
                  <span className="px-2 py-0.5 rounded border border-indigo-900/50 bg-indigo-950/40 text-[10px] text-indigo-300">
                    {s.count} runs
                  </span>
                </li>
              ))}
        ...
        <div className="rounded border border-indigo-900/40 bg-[#0a0a0a]/60 p-5 space-y-3">
          <div className="text-[10px] uppercase tracking-widest text-cyan-400/80 flex items-center gap-1.5 border-b border-indigo-950/60 pb-2">
            ...
            <ul className="space-y-2">
              {metrics.mostUsedTools.map((t) => (
                <li key={t.toolName} className="flex items-center justify-between gap-3">
                  <span className="text-xs text-slate-300 truncate">{t.toolName}</span>
                  <span className="px-2 py-0.5 rounded border border-cyan-900/50 bg-cyan-950/40 text-[10px] text-cyan-300">
                    {t.count} calls
                  </span>
                </li>
              ))}
      ...
      <div className="rounded border border-indigo-900/40 bg-[#0a0a0a]/60 p-5 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-indigo-950/60 pb-3">
          <div className="text-[10px] uppercase tracking-widest text-indigo-400/80 flex items-center gap-1.5">
            <Play className="h-3.5 w-3.5" /> RECENT EXECUTIONS
          </div>
          <label className="relative">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3 w-3 text-indigo-400/70" />
            <input
              type="text"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search executions…"
              className="rounded border border-indigo-900/50 bg-black/50 pl-8 pr-3 py-1.5 text-[11px] text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500/70 transition-all"
            />
          </label>
        </div>
            ...
            <table className="w-full text-left font-mono">
              ...
              <tbody className="divide-y divide-indigo-950/50">
                {executions.map((execution: ExecutionDTO) => (
                  <tr key={execution.id} className="hover:bg-indigo-950/20 transition-colors">
                    ...
                    <td className="py-2.5 pr-3 text-[10px] text-slate-500">{execution.provider ?? "—"}</td>
                    <td className="py-2.5 pr-3 text-[11px] text-slate-400">
                      {execution.durationMs != null ? `${(execution.durationMs / 1000).toFixed(1)}s` : "—"}
                    </td>
                    <td className="py-2.5 text-[10px] text-slate-500">{formatDate(execution.startedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
