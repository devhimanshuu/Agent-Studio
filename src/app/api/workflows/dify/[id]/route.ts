...
export async function GET(
  ...
  try {
    ...
    let dslParsed: any = {};
    try {
      const dslRes = await fetchWithRetry(`https://marketplace.dify.ai/api/v1/templates/${id}/dsl`, {
        timeoutMs: 15000,
        retries: 2,
      });
      if (dslRes.ok) {
    ...
    const convertedGraph = convertDifyToAgentGraph({
      ...
      workflow: dslParsed.workflow || dslParsed,
      app: dslParsed.app,
    ...
    return NextResponse.json(
      {
        success: false,
        error: error?.message || `Failed to fetch Dify template #${id}`,
      },
      { status: 502 }
    );
  }
