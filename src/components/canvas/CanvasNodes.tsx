...
function BaseShell({
  ...
  return (
    <div
      className={clsx(
        "relative w-full rounded border bg-[#0b0b12]/95 font-mono shadow-xl transition-all duration-200",
        accentClass,
        statusClasses(data.traceStatus),
        selected && "ring-2 ring-indigo-400/70"
      )}
    >
      {showTarget && <Handle type="target" position={Position.Left} className="!bg-indigo-400 !border-0 !w-2.5 !h-2.5" />}
      ...
      <div className="p-2.5 space-y-1.5">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="text-[10px] font-bold tracking-wider uppercase text-slate-100 truncate">
              {data.label}
            </span>
          </div>
        ...
        {children}
        {data.traceDetail && (
          <div className="text-[8px] text-slate-400 leading-tight truncate" title={data.traceDetail}>
            {data.traceDetail}
          </div>
        )}
      </div>
...
function AgentNode({ data, selected }: NodePropsShape) {
  ...
  return (
    <BaseShell
      ...
      selected={selected}
    >
      <div className="text-[8px] text-slate-500 leading-tight line-clamp-2" title={data.prompt}>
...
function ToolNode({ data, selected }: NodePropsShape) {
  ...
  return (
    <BaseShell
      ...
      selected={selected}
    >
      <div className="text-[9px] text-cyan-300 font-semibold truncate">{data.toolName ?? "no tool selected"}</div>
      <div className="text-[8px] text-slate-500 truncate">action · {data.action ?? "—"}</div>
    </BaseShell>
  );
}
...
function RouterNode({ data, selected }: NodePropsShape) {
  ...
  return (
    <BaseShell
      ...
      sourceCount={2}
    >
      <div className="text-[8px] text-slate-500 leading-tight line-clamp-2" title={data.condition}>
...
function ApprovalNode({ data, selected }: NodePropsShape) {
  ...
  return (
    <BaseShell
      ...
      selected={selected}
    >
      <div className="text-[8px] text-slate-500 leading-tight line-clamp-2" title={data.approvalReason}>
...
function LoopNode({ data, selected }: NodePropsShape) {
  ...
  return (
    <BaseShell
      ...
      selected={selected}
    >
      <div className="text-[8px] text-slate-500">repeats body edge, then exits</div>
    </BaseShell>
  );
}
...
function StartNode({ data, selected }: NodePropsShape) {
  ...
  return (
    <BaseShell
      ...
      showTarget={false}
    >
      <div className="text-[8px] text-slate-500">entry · user input</div>
    </BaseShell>
  );
}
...
function EndNode({ data, selected }: NodePropsShape) {
  ...
  return (
    <BaseShell
      ...
      showSource={false}
    >
      <div className="text-[8px] text-slate-500">terminal · final output</div>
    </BaseShell>
  );
}
